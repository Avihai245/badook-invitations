...
כשמשתמש שולח, הדף שולח את ההודעות בקבוצות ומראה התקדמות. אם הדף נסגר באמצע, או כשהודעה ממתינה לניסיון חוזר (מגבלת קצב של Meta: היא ממתינה דקה אחרי הניסיון הראשון וחמש דקות אחרי השני), ה-workflow `.github/workflows/whatsapp-queue.yml` רץ כל 10 דקות ושולח את מה שהגיע זמנו. הדף עצמו לא מחכה לניסיונות החוזרים: הוא מסיים ואומר כמה הודעות יישלחו שוב בעוד כמה דקות. הוא צריך שני Secrets ב-GitHub (Settings → Secrets and variables → Actions), אותם Secrets שכבר משמשים לסיכום היומי:
...
- **קרדיט אחד = הודעה אחת.** המחיר שמוצג למשתמש הוא מחיר הודעת Marketing בישראל (`INVITES_WHATSAPP_PRICE_USD` × `INVITES_USD_TO_ILS`, מעוגל כלפי מעלה לאגורה).
