...
| `INVITES_WHATSAPP_PRICE_USD` | המחיר להודעת Marketing בישראל לפי מחירון Meta. ברירת המחדל `0.0353`, וכדאי לבדוק את המחירון העדכני |
...
- **קרדיט אחד = הודעה אחת.** המחיר שמוצג למשתמש הוא מחיר הודעת Marketing בישראל (`INVITES_WHATSAPP_PRICE_USD` × `INVITES_USD_TO_ILS`, מעוגל כלפי מעלה לאגורה). הודעה שנכשלה מחזירה את הקרדיט.
