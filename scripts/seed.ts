...
export function seedSql(): string {
  ...
  for (const { id, manifest, sort } of templates) {
    // an unlisted design (manifest `listed: false`) is seeded inactive: out of the public listing
    lines.push(
      `insert into public.invitation_templates (id, manifest, is_active, sort) values (${lit(id)}, ${json(manifest)}, ${manifest.listed}, ${sort})`,
      `  on conflict (id) do update set manifest = excluded.manifest, sort = excluded.sort, is_active = excluded.is_active;`,
    );
  }
  if (only !== 'templates') {
