...
test.describe('guest list', () => {
  test('Excel upload (twice) → a personal link greets the guest, prefills the form, and the reply is theirs', async ({
    ...
    await expect(dialog.getByText(/^1 מוזמנים עם מספר קווי/)).toBeVisible();
    ...
    await expect(
      actions.getByText(/^3 מוזמנים עוד לא קיבלו · .*0\.14.* להודעה · יתרה: 0 קרדיטים$/),
    ).toBeVisible();
  ...
  test('WhatsApp from the system number: the fixed template, what is skipped, retries that wait, statuses and STOP from the signed webhook', async ({
    ...
    await expect(done).toContainText('1 הודעות לא נשלחו (הקרדיט הוחזר)');
    // WhatsApp's rate limit: tried again later, not three times in a row
    await expect(done).toContainText('1 הודעות יישלחו שוב בעוד כמה דקות');
    ...
    await expect(dialog.getByTestId('whatsapp-plan')).toContainText('יישלחו 1 הודעות');
    ...
    await expect(dialog.getByText(/^3 הודעות × .*0\.14.* = .*0\.42/)).toBeVisible();
  ...
  test('without credits the dialog says how many are missing and does not send (English UI, shekels)', async ({
    ...
    await expect(dialog.getByText('1 messages × ₪0.14 = ₪0.14')).toBeVisible();
    await expect(dialog.getByText('1 credits short')).toBeVisible();
    await dialog.getByRole('checkbox', { name: /I confirm my guests know me/ }).click();
    await expect(dialog.getByRole('button', { name: 'Send to 1 guests' })).toBeDisabled();
