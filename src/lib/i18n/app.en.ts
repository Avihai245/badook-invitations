import type { AppDict } from './app.he';
import { accountEn } from './account.en';
import { billingEn } from './billing.en';
import { guestsEn } from './guests.en';
import { siteEn } from './site.en';
import { helpEn } from './help.en';
import { supportEn } from './support.en';
import { editorEn } from './editor.en';
import { seatingEn } from './seating.en';

/** Host-app UI strings, English. Same shape as app.he.ts (checked by the type). */
export const en: AppDict = {
  brand: 'Badook',
  guests: guestsEn,
  site: siteEn,
  help: helpEn,
  support: supportEn,
  billing: billingEn,
  accountPage: accountEn,
  editor: editorEn,
  seating: seatingEn,
  common: {
    close: 'Close',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    back: 'Back',
    next: 'Continue',
    retry: 'Try again',
    loading: 'Loading…',
    copy: 'Copy',
    copied: 'Copied',
    open: 'Open',
    more: 'More options',
    optional: 'Optional',
    required: 'Required',
    error: 'Something went wrong. Please try again in a moment.',
    hebrew: 'עברית',
    english: 'English',
    both: 'Both',
    toastLabel: 'Notification',
    adults: { one: '1 adult', other: '{n} adults' },
    children: { one: '1 child', other: '{n} children' },
    toastViewport: 'Notifications ({hotkey})',
    helpLabel: 'What the buttons here do',
    helpTitle: 'What each button does',
    people: { one: '1 person', other: '{n} people' },
  },
  errorPages: {
    notFound: {
      metaTitle: 'Page not found',
      code: 'Error 404',
      title: 'We couldn’t find this page',
      body: 'The link may be mistyped, or the invitation was deleted or belongs to another account.',
      toInvitations: 'To my invitations',
      newInvitation: 'New invitation',
      home: 'To the home page',
      login: 'Sign in',
    },
    error: {
      title: 'Something went wrong',
      body: 'It’s on our side, not yours. Please try again in a moment — and if it keeps happening, our assistant or support team will gladly help.',
      retry: 'Try again',
      toInvitations: 'To my invitations',
      contact: 'Contact us',
      code: 'Error code: {code}',
    },
  },
  shell: {
    nav: {
      label: 'Main navigation',
      invitations: 'My invitations',
      newInvitation: 'New invitation',
      billing: 'Plan & billing',
      account: 'My account',
      assistant: 'Ask the assistant',
      contact: 'Contact us',
      help: 'Help & assistant',
    },
    tabs: {
      label: 'Navigation',
      invitations: 'Invitations',
      newInvitation: 'New',
      billing: 'Plan',
      help: 'Help',
    },
    legal: 'Legal & policies',
    signedInAs: 'Signed in as',
    userMenu: 'Account menu',
    uiLanguage: 'Interface language',
    account: 'My account',
    signOut: 'Sign out',
    skipToContent: 'Skip to content',
    invitationNav: {
      label: 'Invitation navigation',
      edit: 'Edit',
      guests: 'Guests',
      responses: 'RSVPs',
      share: 'Share',
      back: 'All invitations',
    },
  },
  workspace: {
    label: 'Invitation navigation',
    back: 'All invitations',
    tabs: {
      overview: 'Overview',
      guests: 'Guests & WhatsApp sending',
      guestsShort: 'Guests & WhatsApp',
      responses: 'RSVPs',
      share: 'Share',
      edit: 'Edit the design',
    },
    editHint: 'The editor opens full screen. “Back” returns to your invitations.',
    publish: 'Publish',
    openInvitation: 'Open the invitation',
    editDesign: 'Edit the design',
    guestsCount: { one: '1 guest', other: '{n} guests' },
    responsesCount: { one: '1 reply', other: '{n} replies' },
  },
  overview: {
    metaTitle: 'Overview · {name}',
    title: 'Overview',
    subtitle: 'What’s happening with your invitation, and what’s next.',
    stats: {
      label: 'Your invitation in numbers',
      guests: 'On the guest list',
      sent: 'Received it',
      attending: 'Coming',
      declined: 'Not coming',
      pending: 'Haven’t replied',
      people: { one: '1 person coming', other: '{n} people coming' },
      ofGuests: 'of {n}',
      noGuests: 'The list is empty',
    },
    main: {
      title: 'What to do now',
      import: {
        title: 'Upload your guest list from Excel',
        body: 'An Excel or CSV file with names and phone numbers. Each guest gets a personal link with their name.',
        cta: 'Upload a file',
        more: 'Add more guests',
      },
      send: {
        title: 'Send on WhatsApp to all your guests',
        body: 'A personal invitation for each guest from our official number — and see who got it, opened it and replied.',
        cta: 'Send on WhatsApp',
        noGuests: 'Upload a guest list first',
        notPublished: 'Publish the invitation first',
        allSent: 'Sent to all your guests',
        toSend: { one: '1 guest hasn’t got it yet', other: '{n} guests haven’t got it yet' },
      },
    },
    steps: {
      title: 'The way to a perfect invitation',
      progress: '{done} of {total}',
      done: 'Done',
      design: {
        title: 'Design the invitation',
        body: 'Names, date, venue, colors and music.',
        cta: 'Edit the design',
      },
      publish: {
        title: 'Publish',
        body: 'The link goes live and you can send it to your guests.',
        cta: 'Publish',
        again: 'There are unpublished changes',
      },
      import: {
        title: 'Guest list from Excel',
        body: 'Names and phones — each guest gets a personal link.',
        cta: 'Upload a list',
      },
      send: {
        title: 'Send on WhatsApp',
        body: 'To your whole list, in one go.',
        cta: 'Send',
      },
      track: {
        title: 'Follow the RSVPs',
        body: 'Who’s coming and with how many, as it happens.',
        cta: 'See the RSVPs',
      },
    },
    link: {
      title: 'The invitation link',
      copy: 'Copy',
      copied: 'Link copied',
      notLive: 'The link starts working once you publish.',
      share: 'More ways to share',
    },
  },
  auth: {
    panelTitle: 'Invitations your guests will love',
    panelBody: 'Design, music and RSVPs — all in one place, in Hebrew and English.',
    loginTitle: 'Sign in',
    loginSubtitle: 'Welcome back! Sign in to edit and share your invitations.',
    signupTitle: 'Create an account',
    signupSubtitle: 'A few seconds and you can start designing an invitation.',
    name: 'Name',
    email: 'Email',
    password: 'Password',
    newPassword: 'New password',
    passwordHint: 'At least 8 characters',
    login: 'Sign in',
    signup: 'Create account',
    noAccount: 'No account yet?',
    haveAccount: 'Already have an account?',
    toSignup: 'Sign up',
    toLogin: 'Sign in',
    forgot: 'Forgot your password?',
    forgotTitle: 'Reset your password',
    forgotSubtitle: 'Enter your account email and we’ll send a link to choose a new password.',
    sendLink: 'Send link',
    linkSent: 'If there is an account with that address, we’ve sent it a password reset link.',
    updateTitle: 'Choose a new password',
    updateSubtitle: 'Enter a new password for your account.',
    updatePassword: 'Save password',
    checkEmailTitle: 'Check your inbox',
    checkEmail: 'We sent a confirmation link to {email}. Click it to get started.',
    google: 'Continue with Google',
    orEmail: 'or with email',
    errors: {
      invalid_credentials: 'Wrong email or password.',
      email_not_confirmed: 'You haven’t confirmed your email yet — look for the link we sent.',
      user_already_exists: 'There’s already an account with this email. Try signing in.',
      weak_password: 'That password is too short — at least 8 characters.',
      invalid_email: 'That email address isn’t valid.',
      rate_limited: 'Too many attempts. Please try again in a few minutes.',
      link_invalid: 'This link is invalid or has expired. Request a new one.',
      oauth_failed:
        'Signing in with Google didn’t finish. Try again, or sign in with your email and password.',
      session_missing: 'To choose a new password, open the link from the email.',
      generic: 'Something went wrong. Please try again in a moment.',
    },
  },
  eventTypes: {
    wedding: 'Wedding',
    engagement: 'Engagement',
    henna: 'Henna',
    bar_mitzvah: 'Bar mitzvah',
    bat_mitzvah: 'Bat mitzvah',
    brit: 'Brit',
    baby_shower: 'Baby shower',
    birthday: 'Birthday',
    save_the_date: 'Save the Date',
    corporate: 'Company event',
    other: 'Other',
  },
  status: {
    draft: 'Draft',
    published: 'Live',
    archived: 'Archived',
    unpublishedChanges: 'Unpublished changes',
  },
  list: {
    title: 'My invitations',
    newInvitation: 'New invitation',
    stats: {
      one: '1 response · {attending} attending',
      other: '{responses} responses · {attending} attending',
    },
    noResponses: 'No responses yet',
    menu: {
      edit: 'Edit',
      share: 'Share',
      guests: 'Guests',
      responses: 'Responses',
      duplicate: 'Duplicate',
      archive: 'Archive',
      unarchive: 'Restore from archive',
      followUp: 'Create the full invitation',
    },
    duplicated: 'Copy created: {slug}',
    archived: 'Invitation archived',
    unarchived: 'Invitation restored',
    showArchived: { one: 'Archive (1)', other: 'Archive ({n})' },
    hideArchived: 'Back to invitations',
    emptyTitle: 'No invitations yet',
    emptyBody: 'Pick a design and you’ll have a moving invitation in minutes',
    emptyCta: 'Choose a design',
    archivedEmpty: 'No archived invitations',
    greeting: 'Hi, {name}',
    greetingNoName: 'Hi there!',
    summaryEmpty: 'Let’s create your first invitation.',
    active: { one: '1 active invitation', other: '{n} active invitations' },
    responsesTotal: { one: '1 response', other: '{n} responses' },
    attendingTotal: { one: '1 attending', other: '{n} attending' },
    statsLabel: 'Your invitations at a glance',
    countdown: {
      today: 'Today!',
      days: { one: 'Tomorrow', two: 'In 2 days', other: 'In {n} days' },
    },
    past: 'The event has passed',
    nextHint: 'The next thing worth doing with this invitation. One tap takes you straight there.',
    archiveHint:
      'Invitations you archived: off the list and not counted toward your plan. Bring one back from its ⋯ menu.',
    hideArchivedHint: 'Back to your active invitations.',
    next: {
      publish: 'Next step: publish',
      republish: 'You have changes: publish again',
      import: 'Next step: upload your guest list',
      send: { one: 'Send on WhatsApp to 1 guest', other: 'Send on WhatsApp to {n} guests' },
      track: 'See who’s coming',
    },
    steps: {
      title: 'How it works',
      design: {
        title: 'Pick a design',
        body: '{n} designs for every kind of event, with an animated opening and music.',
      },
      details: {
        title: 'Fill in the details and publish',
        body: 'Names, date and venue. The texts are already written, and you can change everything.',
      },
      guests: {
        title: 'Upload your guest list',
        body: 'From an Excel file — each guest gets a personal link with their name.',
      },
      send: {
        title: 'Send on WhatsApp and follow',
        body: 'To your whole list in one go, and see who’s coming as it happens.',
      },
    },
    actions: {
      label: 'Quick actions: {name}',
      guests: 'Guests & WhatsApp',
      responses: 'RSVPs',
      share: 'Share',
      edit: 'Edit',
    },
    progress: {
      sent: 'Sent to {sent} of {guests}',
      attending: '{attending} coming',
      noGuests: 'No guest list yet',
    },
    followUp: {
      title: 'The full invitation',
      description:
        'The names, date, languages, design and opening are copied from the save-the-date. The save-the-date stays as it is, and once the new invitation is published it links to it.',
      typeLabel: 'Which event?',
      create: 'Create the invitation',
      creating: 'Preparing the full invitation…',
      error: 'We couldn’t create the invitation. Please try again.',
    },
  },
  home: {
    soon: 'Coming soon: digital invitations with RSVPs — in Hebrew and English.',
    badge: 'Digital invitations in Hebrew and English',
    title: 'Digital invitations',
    titleAccent: 'your guests will love',
    subtitle:
      'Pick a design, add your details and send it on WhatsApp — with an animated opening, music and RSVPs in one place.',
    start: 'Create an invitation',
    login: 'Sign in',
    sample: 'See a sample invitation',
    trust: ['Animated opening & music', 'RSVPs in one place', 'Send on WhatsApp'],
    how: {
      title: 'How it works',
      steps: [
        {
          title: 'Pick a design',
          body: 'Weddings, bar/bat mitzvahs, brits, birthdays and more — each with ready-made texts.',
        },
        {
          title: 'Add your details',
          body: 'Names, date, venue, your own song or video — and watch the invitation change as you type.',
        },
        {
          title: 'Send it, get replies',
          body: 'A WhatsApp link with a beautiful preview, and the RSVPs come straight to you.',
        },
      ],
    },
    designs: {
      title: 'Designs for every occasion',
      subtitle: 'Each comes with an animated opening, music and ready-made texts — all of it editable.',
      cta: 'See all designs',
    },
    features: {
      title: 'Everything an invitation needs',
      opening: {
        title: 'Animated opening & music',
        body: 'An envelope that opens with a tap, a song and a live Hebrew–English switch.',
      },
      rsvp: {
        title: 'RSVPs in one place',
        body: 'Who is coming, how many children, dietary needs — and an Excel export in one click.',
      },
      share: {
        title: 'Send on WhatsApp',
        body: 'A link with a beautiful preview, a ready message and a QR code.',
      },
      video: {
        title: 'A photo or video background',
        body: 'Upload your own photo or video, or paste a YouTube link.',
      },
      languages: {
        title: 'Hebrew and English',
        body: 'One invitation in two languages — guests switch between them with a tap.',
      },
      saveTheDate: {
        title: 'Save the date',
        body: 'Send a save-the-date first, then the full invitation.',
      },
    },
    final: {
      title: 'Ready to invite your guests?',
      body: 'Your first invitation is ready in a few minutes.',
    },
    footer: 'Badook · digital invitations',
  },
  gallery: {
    title: 'Choose a design',
    subtitle: 'Every design comes with an animated opening, music and ready-made texts — all editable.',
    previewLanguage: 'Preview language',
    filterLabel: 'Filter by event type',
    all: 'All',
    results: { one: '1 design', other: '{n} designs' },
    playPreview: 'Play the {name} preview',
    noResults: 'No designs for this event type yet.',
    premium: 'Premium',
    preview: {
      title: '{name}',
      palettes: 'Colors',
      fonts: 'Fonts',
      moreFonts: 'More fonts',
      liveDemo: 'Live demo in a new tab',
      use: 'Use this design',
      phoneLabel: 'Live preview of {name}',
    },
  },
  wizard: {
    title: 'New invitation · {template}',
    progress: 'Step {step} of 3',
    step1: 'What are you celebrating?',
    step2: 'Event details',
    step3: 'Which language?',
    fields: {
      name1: 'Name 1',
      name2: 'Name 2',
      celebrant: 'Celebrant’s name',
      parents: 'Parents’ names',
      parentsHint: 'Shown at the bottom of the invitation',
      name: 'Name',
      age: 'Age',
      ageHint: 'Shown on the opening card',
      babyParents: 'Parents’ names',
      host: 'Host name',
      date: 'Date',
      startTime: 'Time',
      endTime: 'End time',
      timezone: 'Time zone',
      namesIn: { he: 'Names in Hebrew', en: 'Names in English' },
    },
    languages: {
      he: 'עברית',
      en: 'English',
      both: 'Both',
      bothHint: 'Guests can switch between the languages',
      default: 'Default language',
    },
    create: 'Create invitation',
    creating: 'Preparing your invitation…',
    errors: {
      required: 'Required',
      date: 'Pick a date',
      time: 'Pick a time',
      age: 'Age between 1 and 120',
      create: 'We couldn’t create the invitation. Please try again.',
    },
  },
  responses: {
    metaTitle: 'RSVPs · {name}',
    title: 'RSVPs',
    share: 'Share',
    export: 'Export to Excel',
    exportHelp: 'Downloads every reply to a file that opens in Excel, with dietary preferences and answers.',
    exportEmpty: 'No replies to export yet. This works as soon as the first reply arrives.',
    kpi: {
      attending: 'Attending',
      attendingSub: '{adults} · {children}',
      responses: 'Replies',
      responsesSub: '+{n} this week',
      declined: 'Not attending',
      declinedSub: '{pct}% of replies',
      deadline: 'Until the deadline',
      days: { one: '1 day', other: '{n} days' },
      today: 'Today',
      closed: 'Closed',
      none: 'No deadline',
    },
    dietary: 'Dietary preferences',
    search: 'Search by name, phone or email',
    statusFilter: 'Filter by reply',
    all: 'All',
    attending: 'Attending',
    declined: 'Not attending',
    filter: 'Filter',
    withMessage: 'With a message',
    withDietary: 'With dietary preferences',
    clearFilters: 'Clear filters',
    activeFilters: 'Active filters',
    removeFilter: 'Remove the filter: {name}',
    showing: 'Showing {shown} of {total}',
    openReply: 'Details of {name}’s reply',
    columns: {
      name: 'Name',
      status: 'Status',
      adults: 'Adults',
      children: 'Children',
      dietary: 'Dietary',
      message: 'Message',
      received: 'Received',
    },
    tableCaption: 'RSVP replies',
    noMatches: 'No replies match the search',
    emptyTitle: 'No replies yet',
    emptyBody: 'Share the invitation — replies show up here as soon as they arrive.',
    emptyDraftTitle: 'This invitation isn’t published yet',
    emptyDraftBody: 'Once it’s published, guests can RSVP and their replies show up here right away.',
    toPublish: 'Publish the invitation',
    emptyArchivedTitle: 'This invitation is archived',
    emptyArchivedBody:
      'Its link is off for now. You can bring it back from the archive on your invitations list.',
    toList: 'To your invitations',
    notify: {
      label: 'Email notifications',
      each: 'For every reply',
      digest: 'Daily summary',
      off: 'Off',
      saved: 'Notification setting saved',
    },
    drawer: {
      received: 'Received {date}',
      updated: 'Updated {date}',
      contact: 'Contact',
      guests: 'Guests',
      adult: 'Adult',
      child: 'Child',
      age: 'Age {n}',
      answers: 'Answers',
      message: 'Message',
      language: 'Reply language',
      notes: 'Notes: {notes}',
      delete: 'Delete this reply',
      deleteTitle: 'Delete the reply from {name}?',
      deleteBody: 'The reply and its guests will be deleted for good.',
      deleted: 'Reply deleted',
    },
    yes: 'Yes',
    no: 'No',
    csv: {
      name: 'Name',
      status: 'Status',
      adults: 'Adults',
      children: 'Children',
      phone: 'Phone',
      email: 'Email',
      guests: 'Guests',
      dietary: 'Dietary',
      dietaryNotes: 'Dietary notes',
      message: 'Message',
      language: 'Language',
      received: 'Received',
      updated: 'Updated',
    },
    diet: {
      none: 'No preference',
      kosher: 'Kosher',
      kosher_mehadrin: 'Kosher (Mehadrin)',
      vegetarian: 'Vegetarian',
      vegan: 'Vegan',
      gluten_free: 'Gluten free',
      dairy_free: 'Dairy free',
      pescatarian: 'Pescatarian',
      nut_allergy: 'Nut allergy',
      other_allergy: 'Other allergy',
      kids_meal: "Kids' meal",
    },
  },
  email: {
    subjectYes: 'New RSVP: {name} · {guests}',
    subjectNo: 'New reply: {name} · not attending',
    subjectUpdated: 'Reply updated: {name} · {status}',
    guests: { one: '1 guest', other: '{n} guests' },
    attending: 'Attending',
    declined: 'Not attending',
    counts: '{adults} · {children}',
    message: 'Their message:',
    open: 'See all RSVPs',
    footerEach:
      'You get this email because notifications for every reply are on — change it on the RSVP dashboard.',
    digestSubject: {
      one: 'Daily summary: 1 new reply · {title}',
      other: 'Daily summary: {n} new replies · {title}',
    },
    digestIntro: 'Replies since the last summary:',
    footerDigest: 'This is the daily RSVP summary — change it on the RSVP dashboard.',
  },
  share: {
    metaTitle: 'Share · {name}',
    title: 'Your invitation is live 🎉',
    subtitle: 'Share the link — any future update shows up automatically at the same link.',
    edit: 'Edit',
    open: 'Open the invitation',
    unpublishedChanges:
      'Some changes aren’t published yet — publish them from the editor so guests see them.',
    link: 'Invitation link',
    copyLink: 'Copy',
    linkCopied: 'Link copied',
    message: 'Message to send',
    messageLanguage: 'Message language',
    whatsapp: 'Send on WhatsApp',
    copyMessage: 'Copy the message',
    messageCopied: 'Message copied',
    copyLinkHint: 'Copies just the link, to paste anywhere.',
    whatsappHint: 'Opens your WhatsApp with the message ready. Pick people or a group there.',
    copyMessageHint:
      'Copies the whole message with the link — to paste in a text, an email or any other app.',
    copyFailed: 'Couldn’t copy — select the text and copy it yourself.',
    preview: 'How it will look on WhatsApp',
    previewImage: 'The link’s preview image',
    qr: 'QR code',
    qrLabel: 'QR code of the invitation link',
    downloadPng: 'Download the QR code as a PNG image',
    downloadSvg: 'Download the QR code as an SVG file',
    notPublishedTitle: 'This invitation isn’t published yet',
    notPublishedBody:
      'Publish it from the editor — then you’ll find the link, a ready message and a QR code here.',
    toEditor: 'Open the editor',
    toPublish: 'Publish the invitation',
    guestsTitle: 'A personal link for every guest',
    guestsBody:
      'Upload your guest list from Excel — each guest gets a link with their name, a prefilled form, and you see who opened and who replied.',
    toGuests: 'To the guest list',
  },
};
