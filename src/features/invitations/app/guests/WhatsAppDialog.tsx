...
export function WhatsAppDialog({
  ...
  const send = async () => {
    ...
    setPhase('done');
    toast({ title: plural(w.done, done - failed, { n: number(done - failed) }), variant: 'success' });
    onDone();
  };
      ...
      {phase === 'done' ? (
        <div className="flex flex-col items-center gap-3 py-4 text-center" data-testid="whatsapp-done">
          ...
          <p className="text-[15px] font-semibold">
            {plural(w.done, progress.done - progress.failed, { n: number(progress.done - progress.failed) })}
          </p>
          {progress.failed ? (
