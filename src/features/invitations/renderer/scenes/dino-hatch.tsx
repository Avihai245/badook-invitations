...
function Rex({ u }: { u: Url }) {
  return (
    <g>
      ...
      <path d="M50 168c0-50 30-86 80-90l30-2c24-2 44 14 44 36 0 14-8 22-22 24l-30 4c-8 30-30 56-62 56-28 0-40-12-40-28z" fill={u('rex')} />
      ...
      <path d="M114 132c8 4 12 10 10 16" stroke={GREEN_DEEP} strokeWidth="7" fill="none" strokeLinecap="round" />
...
function Stego({ u }: { u: Url }) {
  ...
  return (
    <g>
      {plates.map(([x, y, s], i) => (
        <path key={i} d={`M${x! - s! / 2} ${y! + 12}L${x} ${y! - s!}L${x! + s! / 2} ${y! + 12}Z`} style={{ fill: LAVA }} strokeLinejoin="round" />
      ))}
      <path d="M200 100c20 0 34 10 40 24-18-4-30-4-44 2z" fill={YELLOW} />
      <path d="M236 124l-6 8 10-2z" fill={INK} />
      <path d="M60 106c0-34 36-54 86-54s74 26 70 58c-2 14-14 20-30 20H90c-18 0-30-10-30-24z" fill={u('stego')} />
...
function Egg() {
  return (
    <g>
      ...
      <path d="M14 62l14 8 10-10 12 12 12-12 12 10 14-8" stroke={INK} strokeWidth="2.6" fill="none" strokeLinejoin="round" />
      <path d="M50 6C24 6 8 50 8 76c0 24 18 40 42 40s42-16 42-40C92 50 76 6 50 6z" fill="none" stroke="#E2D6B8" strokeWidth="2" />
    </g>
  );
}
...
function Fern({ tone = GREEN_DEEP }: { tone?: string }) {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    leaves.push(`M${r1(x)} ${y}q${-l * 0.5} ${-6} ${-l} ${-2}q${l * 0.5} ${-6} ${l} ${2}M${r1(x)} ${y}q${l * 0.5} ${-6} ${l} ${-2}q${-l * 0.5} ${-6} ${-l} ${2}`);
  }
  return (
    <g>
      <path d="M60 200C58 150 66 90 58 20" stroke={tone} strokeWidth="3.2" fill="none" strokeLinecap="round" />
...
export default function DinoHatch({ place }: SceneProps) {
  ...
  const rand = rng(4);
  const puffs = Array.from({ length: 5 }, (_, i) => [r1(96 + i * 7 + rand() * 6), r1(40 - i * 16), r1(10 + i * 3)] as const);
  return (
    <>
      ...
      <Layer style={{ background: 'radial-gradient(40cqmin 40cqmin at 14% 9%, rgba(255,236,160,.9), rgba(255,236,160,0) 70%)' }} />
      <Piece vb={[0, 0, 60, 60]} anim="twinkle" style={{ left: cm(8), top: cm(7), width: cm(card ? 10 : 14) }}>
        <circle cx="30" cy="30" r="16" fill={YELLOW} />
        {Array.from({ length: 10 }, (_, i) => {
          const a = (i * 36 * Math.PI) / 180;
          return <path key={i} d={`M${r1(30 + Math.cos(a) * 21)} ${r1(30 + Math.sin(a) * 21)}L${r1(30 + Math.cos(a) * 28)} ${r1(30 + Math.sin(a) * 28)}`} stroke={YELLOW} strokeWidth="3" strokeLinecap="round" />;
        })}
      </Piece>
      {/* volcano with smoke and lava */}
      <Piece vb={[0, 0, 200, 160]} style={{ right: cm(-4), bottom: card ? '26cqh' : '17cqh', width: cm(card ? 34 : 54) }}>
        <g opacity=".95">
          {puffs.map(([x, y, r], i) => (
          ...
          <path d="M100 66c-4 16 2 30-4 44" style={{ stroke: LAVA }} strokeWidth="6" fill="none" strokeLinecap="round" />
          <path d="M20 160L84 52h8L50 160z" fill="#fff" opacity=".12" />
        </g>
      </Piece>
      {/* the valley */}
      <Piece vb={[0, 0, 1200, 300]} fit="xMidYMax slice" style={{ left: 0, bottom: 0, width: '100%', height: card ? '32cqh' : '22cqh' }}>
      ...
      <Piece vb={[0, 0, 120, 200]} style={{ right: cm(-4), bottom: cm(-3), width: cm(card ? 12 : 18), rotate: '10deg' }}>
        <Fern tone={GREEN_DARK} />
      </Piece>
      <Piece vb={[0, 0, 220, 200]} style={{ left: cm(card ? 4 : 2), bottom: card ? '10cqh' : '5cqh', width: cm(card ? 24 : 40) }}>
        <g filter={url('soft')}>
          <Rex u={url} />
        </g>
      </Piece>
      <Piece vb={[0, 0, 240, 160]} style={{ right: cm(card ? 4 : 1), bottom: card ? '8cqh' : '4.5cqh', width: cm(card ? 26 : 42) }}>
        <g filter={url('soft')}>
          <Stego u={url} />
        </g>
      </Piece>
      <Piece vb={[0, 0, 100, 120]} anim="sway" style={{ left: '50%', bottom: card ? '6cqh' : '9cqh', width: cm(card ? 7 : 10), translate: '-50% 0' }}>
        <g filter={url('soft')}>
          <Egg />
        </g>
      </Piece>
