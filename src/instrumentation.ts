/**
 * Runs once when a server starts (Next.js instrumentation): logs the production settings still missing
 * (lib/startup-checks.ts), and brings the database's templates and demo invitations up to date with
 * this deployment's code (features/invitations/server/seed-sync.ts) — a no-op unless they changed.
 * INVITES_SEED_SYNC=false turns the latter off.
 *
...
export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    const { logMissingSettings } = await import('./lib/startup-checks');
    logMissingSettings();
